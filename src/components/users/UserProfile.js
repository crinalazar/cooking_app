...
function User() { 
    ...
    const [numberRecipes, setNumberRecipes] = useState('');
    ...
    async function getUser() {
        ...
        try {
            const rec = axios.get('http://localhost:5000/Users/' + userId).then(res => setUser(res.data));
        } catch(e) {
            console.warn(e);
        }
    }
    ...
    async function editRecipe(e){
        ...
        setDirty(false);
        try {
            const res = await axios(url + recipeId, {
                method: 'PUT',
                data: ({
                    "RecipeName": formDataRecipe.RecipeName,
                    "PreparationTime": formDataRecipe.PreparationTime,
    ...
    async function getNoOfRecipes(){
        const number = await axios(url).then(res => setNumberRecipes(res.data.length));
    }

    async function relateUserRecipe(){
        
        if(isSuccessfull === true){
            try {
               await axios.get(url).then(res => {
                    const rec = axios('http://localhost:5000/UsersRecipes/', {
                        method: 'POST',
                        data: ({
                            "userId": userId,
                            "recipeId": numberRecipes + 1,
                        }),
                    })
                });
                history.push("/");
                    ...
                    <form className = "addRecipeForm" onSubmit={postRecipe}>
                        <div className="formFieldUserProfile"> 
                            ...
                            <input 
                                ...
                                value={formDataRecipe.RecipeName || ' '} 
                                type="text" 
                                id="RecipeName" 
                            />
                        </div>
                        <div className="formFieldUserProfile">
                            ...
                            <input 
                                ...
                                id="PreparationTime" 
                                name="preparation" 
                                value={formDataRecipe.PreparationTime || ' '}
                            />
                        </div>
                        <div className="formFieldUserProfile">
                            ...
                            <input 
                                ...
                                id="CookingTime" 
                                name="cooking" 
                                value={formDataRecipe.CookingTime || ' '}
                            />
                        </div>
                        <div className="formFieldUserProfile">
                        ...
                        <div className="formFieldUserProfile">
                            ...
                            <input 
                                ...
                                id="Vegetarian" 
                                name="vegan" 
                                value={formDataRecipe.Vegetarian || ' '}
                                placeholder= "yes/no"
                            />
                        </div>
                        <div className="formFieldUserProfile">
                        ...
                        <div>
                            ...
                            <button 
                                className="profileFormButtons" 
                                type="button" 
                                onClick={ handleAddIngredients }> More ingredients
                            </button>
                        </div>
                        <div className="formFieldUserProfile">
